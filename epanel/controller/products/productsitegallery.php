...
class productsitegallery
{
	function addForm()
	{
		//echo "SELECT * from gallery where productID = '".(int)$_REQUEST['productID']."' and isFront = 'S'" ;
		$imgSql = ets_db_query("SELECT * from gallery where productID = '".(int)$_REQUEST['productID']."' and isFront = 'S'");
        $imgfiles = "";
		...
		var files = [
		   <?php echo $imgfiles; ?>
            ...
            $('#fileupload').fileupload({
    				url: '<?php echo HTTP_SERVER.WS_ADMIN_ROOT; ?>uploadsitegallery.php?productID=<?php echo (int)$_REQUEST['productID']; ?>'
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload">
        ...
        <td>
			    ...
			    <input name="isFront[]" type="hidden" value="S" class="form-control">
			</label>
	   </td>
    ...
    <tr class="template-download">
        ...
        <td>
         ...
         <td>
			    ...
			    <input name="isFront[]" type="hidden" value="S">
			</label>
	   </td>
...
<?php
	...
	{		
		...
		if(ets_db_num_rows($qry) > 0){
			...
			$form->addElement(new Element_Hidden("action", "productsitegallery"));
			...
			$form->addElement(new Element_Hidden("isFront", 'S'));
		...
		if($updimg){
			...
			else
			{
				...
				if(!empty($upload))
				{
					...
					->makecopy('shrink',$target_path,DIR_FS_PROJECT_PATH.$_POST['productID']."/".$title.'_mobile.jpg');
			        $thumb_target_path = DIR_FS_PROJECT_PATH.$_POST['productID']."/thumbnail/";
                    if(!file_exists($thumb_target_path))
                    {
                        mkdir($thumb_target_path);
                        exec("chown ".FILEUSER.FILEUSER." ".$path);
                        exec("chmod 0777 ".$thumb_target_path);
                    }
                    $mresult = $image->shrink(array("height"=>60, "width"=>80),true)
					->makecopy('shrink',$target_path,$thumb_target_path.$title);
				}
			}
		}
		if(!$error){
			/*
			echo "update gallery set 
			username = '".$username."',
...
<script>
$(document).ready(function() {
	var listURL = "helperfunc/projectsitegalleryList.php?productID=<?php echo (int)$_REQUEST['productID'];?>";
