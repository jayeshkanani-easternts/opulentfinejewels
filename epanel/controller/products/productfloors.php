...
class productfloors
{
	function addForm()
	{
		...
		$imgSql = ets_db_query("Select * from gallery where productID = '".(int)$_REQUEST['productID']."' and isFront = 'D' ");
		$imgfiles = "";
		...
		var files = [
		   <?php echo $imgfiles; ?>
			...
			$('#fileupload').fileupload({
    				url: '<?php echo HTTP_SERVER.WS_ADMIN_ROOT; ?>uploadfloor.php?productID=<?php echo (int)$_REQUEST['productID']; ?>'
	...
	<form id="fileupload" action="<?php echo HTTP_SERVER.WS_ADMIN_ROOT; ?>upload.php?productID=<?php echo (int)$_REQUEST['productID']; ?>&isFront=D" method="POST" enctype="multipart/form-data">
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-upload">
        ...
        <td>
			    ...
			    <input name="isFront[]" type="hidden" value="D" class="form-control">
			</label>
	   </td>
    ...
    <tr class="template-download">
        ...
        <td>
         ...
         <td>
			    ...
			    <input name="isFront[]" type="hidden" value="D">
			</label>
	   </td>
...
<?php
	...
	{		
		...
		if(ets_db_num_rows($qry) > 0){
			...
			$form->addElement(new Element_Hidden("action", "productfloors"));
			...
			$form->addElement(new Element_Hidden("isFront", 'D'));
...
<script>
$(document).ready(function() {
	var listURL = "helperfunc/projectfloorList.php?productID=<?php echo (int)$_REQUEST['productID'];?>";
