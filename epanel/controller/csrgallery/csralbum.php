...
class csralbum
{
	...
	{    
		...
		$form->addElement(new Element_Hidden("action", "csralbum"));
		...
		$form->addElement(new Element_HTML('<label class="control-label">Last Sort Order : '.get_last_sortorder('csr_album').'</label><br><br>'));						
	...
	function editForm()
	{		
	    $sql = "select * from csr_album where a_id = '".$_REQUEST['a_id']."'";
		...
		if(ets_db_num_rows($qry) > 0){
			...
			$form->addElement(new Element_HTML("<legend>Edit CSR Album</legend>"));
			$form->addElement(new Element_Hidden("controller", "csrgallery"));
			$form->addElement(new Element_Hidden("action", "csralbum"));
	...
	{   
		...
		$path = DIR_FS_CSR_GALLERY_PATH.$album_id.'/';
		if(!file_exists($path))	{
			mkdir($path);
			//exec("chown ".FILEUSER.FILEUSER." ".$path);
			//exec("chmod 0777 ".$path);
		}
		insSeoLnk($album_id,"csrgallery",$album_slug);
		return true;
	}
	function edit()
	{  
		...
		if(ets_db_query($updqry)){
			updSeoLnk($_POST['a_id'],"csrgallery",$album_slug);
			$album_id = $_POST['a_id'];
			$path = DIR_FS_CSR_GALLERY_PATH.$album_id.'/';
			if(!file_exists($path))	{
				mkdir($path);
	...
	function delete()
	{	
		$sqlAtype = ets_db_query("select * from csr_album where a_id =  '".$_GET['a_id']."'") or die(ets_db_error());
		$albumarr = ets_db_fetch_array($sqlAtype);
		$seo_mod = str_replace(" ","-",getalbumtypeName($albumarr['album_type_id']));
		
		delSeoLnk($_GET['a_id'],'csrgallery');
		$delsql = "Delete from csr_album where a_id= '".$_GET['a_id']."'";
		$delqry = ets_db_query($delsql) or die(ets_db_error().$delsql);
		
		delete_folder(DIR_FS_CSR_GALLERY_PATH.$_GET['a_id']);
		
		$qry = "Delete from csr_gallery_master where a_id = '".$_GET['a_id']."'";
...
<script>
	$(document).ready(function() {
	var listURL = 'helperfunc/csralbumList.php';
	$('#albumlist').dataTable({
	 ...
	 "ajax" : listURL
    	...
    	$('.esorder').editable({params:{"tblName":"csr_album"}});
		$('.estatus').editable({
			params:{"tblName":"csr_album"},
			value: 1,
